...
Variant: bn256-mxfp8-cuda-mma-tma
...
This variant keeps TK tile/global-layout types and the epilogue store path, but
implements A/B value TMA, MXFP8 scale movement, TMEM allocation, mbarriers, and
MXFP8 MMA with explicit CUDA inline PTX:
...
def pack_tk_globals(A, A_sc, B, B_sc, C, M: int, N: int, K: int) -> bytes:
    ...
    blob[2 * TK_GL_SIZE:3 * TK_GL_SIZE] = pack_tk_matrix_gl(
        ptr=B.data_ptr(), rows=N, cols=K, tma_desc=b_tma)
...
for (M, N, K) in [(SHAPE_N, SHAPE_N, SHAPE_N)]:
    ...
    print(f"{ok}  MXFP8 CUDA-MMA-TMA    M=N=K={M:>5}   grid={grid[0]} CTAs ({CTA_GROUP}-CTA clusters)   rel err={rel:.2%}")
