...
Variant: bn256-mxfp8-cuda-cp-tma
...
This variant keeps the TK TMA/MMA/epilogue scaffolding but implements the
SMEM->TMEM MXFP8 scale copy with explicit CUDA inline PTX:
...
for (M, N, K) in [(SHAPE_N, SHAPE_N, SHAPE_N)]:
    ...
    print(f"{ok}  MXFP8 CUDA-CP  M=N=K={M:>5}   grid={grid[0]} CTAs ({CTA_GROUP}-CTA clusters)   rel err={rel:.2%}")
