...
Variant: bn256-mxfp8-tk-tma dense AB value-layout check
Config: BM=128 BN=256 BK=128 NS=5 GROUP_SIZE_M=8 NUM_WARPS=4

Run with:  python <this file>.py   (kernel_ab_dense.cu must sit alongside it)
...
# ── User-tunable constants (mirror kernel_ab_dense.cu) ──
...
TK_GL_SIZE = 384
TK_GLOBALS_SIZE = 5 * TK_GL_SIZE
TK_TMA_DESC_OFFSET = 128


def encode_tk_tile_tensor_map(*, gptr: int, rows: int, cols: int,
                              tile_rows: int, tile_cols: int,
                              elem_bytes: int, dtype: int) -> np.ndarray:
    """Build the 5D CUtensorMap layout used by TK swizzled shared tiles."""
...
def encode_tk_scale_tensor_map(*, gptr: int, outer: int, k_tiles: int) -> np.ndarray:
    """Build the 4D CUtensorMap for `gl<fp8e8m0, -1, -1, 32, 16, sf_tile>`."""
    return encode_tensor_map(
        dtype=TMA_UINT8,
        rank=4,
        gptr=gptr,
...
def pack_tk_matrix_gl(*, ptr: int, rows: int, cols: int, tma_desc: np.ndarray) -> bytes:
    """Pack `gl<T, 1, 1, -1, -1, tile>`."""
    blob = bytearray(TK_GL_SIZE)
    struct.pack_into("Q", blob, 0, int(ptr))
    struct.pack_into("Q", blob, 16, int(rows))
    struct.pack_into("Q", blob, 24, int(cols))
    blob[TK_TMA_DESC_OFFSET:TK_TMA_DESC_OFFSET + 128] = tma_desc.tobytes()
    return bytes(blob)


def pack_tk_scale_gl(*, ptr: int, outer: int, k_tiles: int, tma_desc: np.ndarray) -> bytes:
    """Pack `gl<fp8e8m0, -1, -1, 32, 16, sf_tile>`."""
    blob = bytearray(TK_GL_SIZE)
    struct.pack_into("Q", blob, 0, int(ptr))
    struct.pack_into("Q", blob, 8, int(outer))
    struct.pack_into("Q", blob, 16, int(k_tiles))
    blob[TK_TMA_DESC_OFFSET:TK_TMA_DESC_OFFSET + 128] = tma_desc.tobytes()
    return bytes(blob)


def pack_tk_globals(A, A_sc, B, B_sc, C, M: int, N: int, K: int) -> bytes:
    a_tma = encode_tk_tile_tensor_map(
        gptr=A.data_ptr(), rows=M, cols=K, tile_rows=BM, tile_cols=BK,
        elem_bytes=FP8_BYTES, dtype=TMA_UINT8)
    a_sc_tma = encode_tk_scale_tensor_map(
        gptr=A_sc.data_ptr(), outer=M // BM, k_tiles=K // BK)
    b_tma = encode_tk_tile_tensor_map(
        gptr=B.data_ptr(), rows=K, cols=N, tile_rows=BK, tile_cols=BN_LOCAL,
        elem_bytes=FP8_BYTES, dtype=TMA_UINT8)
    b_sc_tma = encode_tk_scale_tensor_map(
        gptr=B_sc.data_ptr(), outer=N // BN_LOCAL, k_tiles=K // BK)
    d_tma = encode_tk_tile_tensor_map(
        gptr=C.data_ptr(), rows=M, cols=N, tile_rows=BM, tile_cols=STORE_N,
        elem_bytes=BF16_BYTES, dtype=TMA_BFLOAT16)

    blob = bytearray(TK_GLOBALS_SIZE)
    blob[0:TK_GL_SIZE] = pack_tk_matrix_gl(
        ptr=A.data_ptr(), rows=M, cols=K, tma_desc=a_tma)
    blob[TK_GL_SIZE:2 * TK_GL_SIZE] = pack_tk_scale_gl(
        ptr=A_sc.data_ptr(), outer=M // BM, k_tiles=K // BK, tma_desc=a_sc_tma)
    blob[2 * TK_GL_SIZE:3 * TK_GL_SIZE] = pack_tk_matrix_gl(
        ptr=B.data_ptr(), rows=K, cols=N, tma_desc=b_tma)
    blob[3 * TK_GL_SIZE:4 * TK_GL_SIZE] = pack_tk_scale_gl(
        ptr=B_sc.data_ptr(), outer=N // BN_LOCAL, k_tiles=K // BK, tma_desc=b_sc_tma)
    blob[4 * TK_GL_SIZE:5 * TK_GL_SIZE] = pack_tk_matrix_gl(
        ptr=C.data_ptr(), rows=M, cols=N, tma_desc=d_tma)
    return bytes(blob)
...
mod, fns = compile_kernel(os.path.join(HERE, "kernel_ab_dense.cu"),
...
def mxfp8_quantize_b_kn(v: torch.Tensor):
    """BF16/FP32 B[K, N] -> E4M3 B[K, N] plus E8M0 scales [N, K/32]."""
    assert v.dim() == 2
    K, _ = v.shape
    assert K % 32 == 0
    q_nk, e8m0_nk32 = mxfp8_quantize(v.t().contiguous())
    return q_nk.t().contiguous(), e8m0_nk32
...
def setup(M, N, K):
    ...
    if os.environ.get("UNIT_SCALES", "0") == "1":
        A = A_bf16.to(torch.float8_e4m3fn).contiguous()
        B = B_bf16.to(torch.float8_e4m3fn).contiguous()
        A_sc_unsw = torch.full((M, K // 32), int(MXFP8_BIAS), dtype=torch.uint8, device="cuda")
        B_sc_unsw = torch.full((N, K // 32), int(MXFP8_BIAS), dtype=torch.uint8, device="cuda")
    else:
        A, A_sc_unsw = mxfp8_quantize(A_bf16)
        B, B_sc_unsw = mxfp8_quantize_b_kn(B_bf16)
    A_sc = scale_swizzle(A_sc_unsw)
    B_sc = scale_swizzle(B_sc_unsw)
    C = torch.zeros(M, N, dtype=torch.bfloat16, device="cuda")

    globals_blob = pack_tk_globals(A, A_sc, B, B_sc, C, M, N, K)
    arg_globals = (ctypes.c_byte * TK_GLOBALS_SIZE).from_buffer_copy(globals_blob)
    arg_c = ctypes.c_void_p(C.data_ptr())
    args = [arg_globals, arg_c, ctypes.c_int(M), ctypes.c_int(N), ctypes.c_int(K)]
...
for (M, N, K) in [(SHAPE_N, SHAPE_N, SHAPE_N)]:
    ...
    B_ref = mxfp8_dequant(B.t().contiguous(), B_sc_unsw).t()
    C_ref = (mxfp8_dequant(A, A_sc_unsw) @ B_ref).to(torch.bfloat16)
    rel = (C.float() - C_ref.float()).abs().max().item() / C_ref.float().abs().max().item()
    C_ref_abt = (mxfp8_dequant(A, A_sc_unsw) @ B_ref.t()).to(torch.bfloat16)
    rel_abt = (C.float() - C_ref_abt.float()).abs().max().item() / C_ref_abt.float().abs().max().item()
    ...
    print(f"{ok}  dense FP8 AB B[K,N]  M=N=K={M:>5}   grid={grid[0]} CTAs ({CTA_GROUP}-CTA clusters)   rel err={rel:.2%}   rel ABt={rel_abt:.2%}")
