...
Variant: bn256-mxfp8-tk-tma AB experiment
Config: BM=128 BN=256 BK=128 NS=5 GROUP_SIZE_M=8 NUM_WARPS=4

Run with:  python <this file>.py   (kernel_ab.cu must sit alongside it)
...
# ── User-tunable constants (mirror kernel_ab.cu) ──
...
mod, fns = compile_kernel(os.path.join(HERE, "kernel_ab.cu"),
...
for (M, N, K) in [(SHAPE_N, SHAPE_N, SHAPE_N)]:
    ...
    print(f"{ok}  MXFP8 AB B[K,N]  M=N=K={M:>5}   grid={grid[0]} CTAs ({CTA_GROUP}-CTA clusters)   rel err={rel:.2%}")
