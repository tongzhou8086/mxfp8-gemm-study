...
// AB experiment: B values are stored as physical [K, N] row-major, while SFB
// keeps the same Blackwell [N/128, K/128, 32, 16] scale-atom packing.
...
using b_tile = kittens::st_fp8e4m3<BK, BN_LOCAL>;
...
struct TkMxfp8Globals {
    ...
    b_gl B;       // B[K, N], FP8 E4M3, consumed by AB MMA
    sf_gl B_sc;   // swizzled E8M0 scales [N/128, K/128, 32, 16]
    d_gl D;       // D[M, N], BF16
};

template<typename D, typename AB, typename SAB, int M, int N, int scale_factor_id>
__device__ static inline constexpr uint32_t mxfp8_ab_instruction_descriptor() {
    static_assert(std::is_same_v<D, float>, "MXFP8 block-scaled MMA accumulates to FP32.");
    static_assert(std::is_same_v<AB, kittens::fp8e4m3>, "This AB test is only for MXFP8 E4M3 values.");
    static_assert(std::is_same_v<SAB, kittens::fp8e8m0>, "This AB test is only for E8M0 scales.");

    uint32_t desc = 0;
    desc |= scale_factor_id << 4;       // Matrix B Scale Factor ID
    desc |= 0b000 << 7;                 // Matrix A is E4M3
    desc |= 0b000 << 10;                // Matrix B is E4M3
    desc |= 0b0 << 13;                  // Do not negate A
    desc |= 0b0 << 14;                  // Do not negate B
    desc |= 0b0 << 15;                  // A tile is not transposed
    desc |= 0b1 << 16;                  // B tile is MN-major / logical AB
    desc |= (N >> 3) << 17;             // B matrix N dimension
    desc |= 0b1 << 23;                  // Scale type is UE8M0
    desc |= (M >> 7) << 27;             // A matrix M dimension
    desc |= scale_factor_id << 29;      // Matrix A Scale Factor ID
    return desc;
}

template<
    int acc,
    kittens::ducks::tt::all D,
    kittens::ducks::st_descriptor::input A,
    kittens::ducks::st_descriptor::input B,
    kittens::ducks::tt::all SA,
    kittens::ducks::tt::all SB
>
__device__ static inline void mma2_AB_mxfp8(
    D& d, const A& a, const B& b, const SA& sa, const SB& sb,
    ...
    static_assert(std::is_same_v<typename SA::T, typename SB::T>);
    static_assert(std::is_same_v<T_AB, kittens::fp8e4m3>);
    static_assert(std::is_same_v<T_SAB, kittens::fp8e8m0>);
    static_assert(std::is_same_v<T_D, float>);

    constexpr int ncta = CTA_GROUP;
    constexpr int M = A::rows * ncta;
    constexpr int N = B::cols * ncta;
    constexpr int K = A::cols;
    static_assert(ncta == 2);
    static_assert(B::rows == K);
    static_assert(K % 32 == 0);
    static_assert(M == D::rows * ncta);
    static_assert(N == D::cols);

    kittens::st_descriptor<kittens::ducks::st_descriptor::detail::get_st<A>, kittens::transpose::N> a_desc(a);
    kittens::st_descriptor<kittens::ducks::st_descriptor::detail::get_st<B>, kittens::transpose::T> b_desc(b);

    constexpr uint32_t idescs[4] = {
        mxfp8_ab_instruction_descriptor<T_D, T_AB, T_SAB, M, N, 0>(),
        mxfp8_ab_instruction_descriptor<T_D, T_AB, T_SAB, M, N, 1>(),
        mxfp8_ab_instruction_descriptor<T_D, T_AB, T_SAB, M, N, 2>(),
        mxfp8_ab_instruction_descriptor<T_D, T_AB, T_SAB, M, N, 3>()
    };

    constexpr int M_offset = M / 32 / ncta;
    constexpr int N_offset = N / 32;

    kittens::detail::tcgen05::template st_st<T_AB, T_SAB, acc, ncta, 32>(
        d.addr,
    ...
    for (int i = 1; i < K / 32; i++) {
        kittens::detail::tcgen05::template st_st<T_AB, T_SAB, 1, ncta, 32>(
            d.addr,
            a_desc.chunk_descriptor(i),
            b_desc.chunk_descriptor(i),
            sa.addr + (i >> 2) * M_offset,
            sb.addr + (i >> 2) * N_offset,
            idescs[i % 4]
        );
    }
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        for (int ti = 0; ti < num_my; ti++) {
            ...
            for (int k = 0; k < num_k; k++) {
                ...
                kittens::tma::cluster::load_async(
                    b_smem[slot], g.B, {k, local_n / BN_LOCAL},
                    mbar_compute_data_ready[slot],
                    static_cast<uint16_t>(1 << cta_rank), 0);
                kittens::tma::cluster::load_async(
        ...
        for (int ti = 0; ti < num_my; ti++) {
            ...
            for (int k = 0; k < num_k; k++) {
                ...
                ::load_mxnv_scale_async2(a_sc_stage, a_sc_smem[slot]);
                ::load_mxnv_scale_async2(b_sc_stage_0, b_sc_smem[slot][0]);
                ::load_mxnv_scale_async2(b_sc_stage_1, b_sc_smem[slot][1]);

                auto b_sc_stage = b_sc_tm.template subtile<kittens::full_tt_fp8e8m0<32>>(slot * 32);
                if (k == 0) {
                    kittens::mm2_AB(out_tm, a_smem[slot], b_smem[slot],
                                    mbar_compute_buffer_free[slot]);
                } else {
                    kittens::mma2_AB(out_tm, a_smem[slot], b_smem[slot],
                                     mbar_compute_buffer_free[slot]);
                }
