...
Tier: bn256-double-tmem-ns6-tk-tma-managed
...
def pack_tk_gl(*, ptr: int, rows: int, cols: int, tma_desc: np.ndarray) -> bytes:
    """Pack `gl<bf16, 1, 1, -1, -1, tile>` as used by this kernel.

    Layout was checked against C++:
...
def pack_tk_globals(A, B, C, M: int, N: int, K: int) -> bytes:
    ...
    blob[0:TK_GL_SIZE] = pack_tk_gl(
        ptr=A.data_ptr(), rows=M, cols=K, tma_desc=a_tma)
    blob[TK_GL_SIZE:2 * TK_GL_SIZE] = pack_tk_gl(
        ptr=B.data_ptr(), rows=K, cols=N, tma_desc=b_tma)
    blob[2 * TK_GL_SIZE:3 * TK_GL_SIZE] = pack_tk_gl(
...
def setup(M, N, K):
    ...
    globals_blob = pack_tk_globals(A, B, C, M, N, K)
