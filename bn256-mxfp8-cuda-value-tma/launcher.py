...
Variant: bn256-mxfp8-cuda-value-tma
...
This variant keeps the TK MMA/TMEM/epilogue scaffolding but implements the
A/B value TMA and MXFP8 scale path with explicit CUDA inline PTX:
...
for (M, N, K) in [(SHAPE_N, SHAPE_N, SHAPE_N)]:
    ...
    print(f"{ok}  MXFP8 CUDA-VALUE-TMA  M=N=K={M:>5}   grid={grid[0]} CTAs ({CTA_GROUP}-CTA clusters)   rel err={rel:.2%}")
